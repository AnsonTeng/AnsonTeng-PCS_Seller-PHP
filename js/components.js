...
setTimeout(function () {
    $(".tab-panel > div:first-of-type").show();
}, 100);
...
for (i = 0; i < accordionebox.length; i++) {
    accordionebox[i].onclick = function () {
        ...
        if (panel.style.maxHeight) {
            ...
            $(this)
                .find(".sea-ui.switch-toggle.mainToggle")
                .show();
        } else {
...
$(window).scroll(function (event) {
    ...
    var $sellerActionBar = $(".seller-action-bar");

    if (scroll <= $(".breadcrumb-back").offset().top) {
        $sellerActionBar.removeClass("float-bar");
        $sellerActionBar.removeClass("shrink-float-bar");
    } else if (scroll > $(".breadcrumb-back").offset().top) {
        if (scroll >= last) {
            $sellerActionBar.addClass("shrink-float-bar");
            $sellerActionBar.removeClass("float-bar");
        } else {
            $sellerActionBar.addClass("float-bar");
            $sellerActionBar.removeClass("shrink-float-bar");
        }
    }
});

<!-- 購買多件商品 -->
<div id="pct-lightbox-buymany" class="pct-ui modal">
    <i class="pct-icon icon-delete lightbox_close"></i>
    <div class="modal-content">
        <div class="description">
            <div class="title">購買多件商品</div>
            <p>此則訊息為【買多件】需求，建議您可以為買家欲購買數量重新設計商品賣場，依照多件商品重新定義包裹重量和材積大小可以幫助買家大幅減少國際運費。設定完畢後，請再將新賣場連結與發問者分享。</p>
        </div>
    </div>
</div>




<!-- 回覆評價 -->
<div id="pct-lightbox-replyreview" class="pct-ui modal medium reviewlist">
    <!--說明 FOR RD: 若送出需較長時間請執行以下loading-->
    <div class="pct-ui loading-overlay" style="display: none;">
        <div class="loading-circle small"></div>
    </div>
    <i class="pct-icon icon-delete lightbox_close"></i>
    <div class="modal-header">回覆評價</div>
    <div class="scrolling-tall modal-content">
        <div class="description pct-reverse">

            <div>
                <div class="description-list">
                    <div class="description-list-box">
                        <div class="list-box-item">買家</div> 
                        <div class="list-box-text eng-num">sophia07926@gmail.com</div>    
                    </div> 
                    <div class="description-list-box">
                        <div class="list-box-item">評價日期</div> 
                        <div class="list-box-text eng-num">2018/05/28  01:28 </div>    
                    </div> 
                    <div class="description-list-box">
                        <div class="list-box-item">評價分數</div> 
                        <div class="list-box-text">
                            <div class="pct-ui review-star">
                                <ul>
                                    <li class="pct-icon icon-star-on"></li>
                                    <li class="pct-icon icon-star-on"></li>
                                    <li class="pct-icon icon-star-on"></li>
                                    <li class="pct-icon icon-star-on"></li>
                                    <li class="pct-icon icon-star-half"></li>
                                </ul>
                                <div class="review-text eng-num">4.5</div>
                            </div>
                            <div class="review-level-text">優良</div>
                        </div>    
                    </div> 
                    <div class="description-list-box">
                        <div class="list-box-item">評價內容</div> 
                        <div class="list-box-text">盒子很有質感，出貨速度很快。收到貨時很喜歡，過程中賣家回覆很及時，也很有耐心回答商品相關問題。下次還會繼續再購買!</div>    
                    </div> 
                </div>          
            </div>
            
            <div>
                <div class="pct-ui form-select-label break-label">
                    <label for="">預設回覆內容</label>
                    <div class="pct-ui form select">
                        <select>
                            <option value="0">請選擇</option>
                            <option value="1">option01</option>
                            <option value="2">option02</option>
                            <option value="3">option03</option>
                            <option value="4">option04</option>
                        </select>
                    </div>
               </div>
               <div class="pct-ui form-label break-label">
                    <label for="">回覆買家評價</label>
                    <!--說明FOR RD: 錯誤訊息樣式請帶 error class="pct-ui form error" -->
                     <div class="pct-ui form">
                    <textarea placeholder="請輸入您要回覆給買家的評價"></textarea>

                    <!--說明FOR RD: 錯誤訊息文字區塊如下 -->
                    <div class="error-notification-tooltip-inline" style="display: none;">送出前請填寫回覆訊息</div>
                    </div>
               </div>
                <div class="title">說明</div>
                <p> 1. 回覆內容一旦送出便無法修改刪除，送出前請仔細考慮。</p>
                <p> 2. 所有評價意見都會在您的賣場頁面呈現，供其他客人參考給予。</p>
                <p> 3. 每筆交易僅限更新評價紀錄一次。系統只會以最新一次評價作為分數計算。</p>
           </div>

        </div>
    </div>
    <div class="modal-actions">
        <a href="#" class="pct-ui btn btn-cta-outlined">取消重填</a>
        <a href="#" class="pct-ui btn btn-cta">確定送出</a>
    </div>
</div>

<!-- 過去評價 -->
<div id="pct-lightbox-historyreview" class="pct-ui modal medium reviewlist">
    <i class="pct-icon icon-delete lightbox_close"></i>
    <div class="modal-header">過去評價</div>
    <div class="scrolling-tall modal-content">
        <div class="description">
           <div class="title">買家上一次評價紀錄</div>
           <div class="description-list">
                <div class="description-list-box">
                    <div class="list-box-item">買家</div> 
                    <div class="list-box-text eng-num">sophia07926@gmail.com</div>    
                </div> 
                <div class="description-list-box">
                    <div class="list-box-item">評價日期</div> 
                    <div class="list-box-text eng-num">2018/05/28  01:28 </div>    
                </div> 
                <div class="description-list-box">
                    <div class="list-box-item">評價分數</div> 
                    <div class="list-box-text">
                        <div class="pct-ui review-star">
                            <ul>
                                <li class="pct-icon icon-star-on"></li>
                                <li class="pct-icon icon-star-on"></li>
                                <li class="pct-icon icon-star-on"></li>
                                <li class="pct-icon icon-star-on"></li>
                                <li class="pct-icon icon-star-half"></li>
                            </ul>
                            <div class="review-text eng-num">4.5</div>
                        </div>
                        <div class="review-level-text">優良</div>
                    </div>    
                </div> 
                <div class="description-list-box">
                    <div class="list-box-item">評價內容</div> 
                    <div class="list-box-text">盒子很有質感，出貨速度很快。收到貨時很喜歡，過程中賣家回覆很及時，也很有耐心回答商品相關問題。下次還會繼續再購買!</div>    
                </div> 
           </div>
            

            <div class="title">賣家回覆紀錄</div>
           <div class="description-list">
                <div class="description-list-box">
                    <div class="list-box-item">回覆日期</div> 
                    <div class="list-box-text eng-num">2018/05/28  01:28 </div>    
                </div> 
                <div class="description-list-box">
                    <div class="list-box-item">賣家回覆</div> 
                    <div class="list-box-text">匯款迅速的好買家，很愉快！</div>    
                </div> 
           </div>
        </div>
    </div>
</div>
